...
#Region "On Init"
    ...
    Protected Overrides Sub OnInitComplete(ByVal e As System.EventArgs)
        ...
        Dim Sql As String = " SELECT   distinct  Project.Proj_id, Project.Proj_Title " _
                            & " FROM         Project INNER JOIN Project_Needs ON Project_Needs.proj_proj_id = Project.Proj_id " _
                            & " WHERE     (Project_Needs.approved_amount IS NULL or Project_Needs.approved_amount <= 0) "
...
#Region "Fills"
    Private Sub FillGrid()
        ...
        sql = " select Project_Needs_Document.File_name,dbo.Project_Needs.PNed_ID, dbo.Project_Needs.PNed_Name, dbo.Project_Needs.PNed_desc, dbo.Project_Needs.PNed_Number, " _
          ...
          & " dbo.Project_Needs_Document ON dbo.Project_Needs.Need_Doc_id = dbo.Project_Needs_Document.id"
        If (Smart_Search_Proj.SelectedValue <> "0" And Smart_Search_Proj.SelectedValue <> "") Then
            sql += " where proj_proj_id = " & Smart_Search_Proj.SelectedValue _
...
#Region "save"
    Protected Sub saveAll_Click(ByVal sender As Object, ByVal e As EventArgs) Handles saveAll.Click
        ...
        sql = "select Project_Needs.* ,NMT_Desc,NST_Desc from Project_Needs" _
            ...
            & " where proj_proj_id = " & Smart_Search_Proj.SelectedValue _
            & " order by NMT_Desc,NST_Desc,PNed_ID"
        Dim dt1 As New DataTable
        dt1 = General_Helping.GetDataTable(sql)
        For Each row As GridViewRow In gvMain.Rows
...
#End Region
    ...
    Public Sub fillgridDoc()
        ...
        Dim sql1 As String = ""
        sql1 = " SELECT DISTINCT Project_Needs_Document.id,Project_Needs_Document.[File_name] FROM dbo.Project_Needs_Document  JOIN dbo.Project_Needs ON  dbo.Project_Needs_Document.id =dbo.Project_Needs.Need_Doc_id  "
      
        If (Smart_Search_Proj.SelectedValue <> "0" And Smart_Search_Proj.SelectedValue <> "") Then
            sql1 += " where Project_Needs.proj_proj_id = " & Smart_Search_Proj.SelectedValue

        End If
        Dim dt11 As New DataTable
        dt11 = General_Helping.GetDataTable(sql1)

        GridNeed.DataSource = dt11
        GridNeed.DataBind()


        'Dim cmd As New SqlCommand("SELECT DISTINCT Project_Needs_Document.id,Project_Needs_Document.[File_name] FROM dbo.Project_Needs_Document  JOIN dbo.Project_Needs ON  dbo.Project_Needs_Document.id =dbo.Project_Needs.Need_Doc_id  where Project_Needs.proj_proj_id = " & Smart_Search_Proj.SelectedValue, con)
        'cmd.Connection = con
        'Dim sqladptr As SqlDataAdapter = New SqlDataAdapter
        'sqladptr.SelectCommand = cmd
        'Dim DT As DataTable = New DataTable()
